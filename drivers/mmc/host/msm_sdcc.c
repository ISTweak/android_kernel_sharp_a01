...
static struct semaphore	sdio_suspendresume_sem;
struct mmc_host *msmsdcc_idle_mmc0;
unsigned int  msmsdcc_state_suspend = 0;
unsigned int  msmsdcc_state_idle = 0;
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if( strncmp(mmc_hostname(mmc),"mmc0",4) == 0){
		...
		init_MUTEX(&sdio_suspendresume_sem);
	...
	if (mmc) {
		...
		if( strncmp(mmc_hostname(mmc),"mmc0",4) != 0){
			...
			if ((mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ) && release_lock)
				wake_lock_timeout(&host->sdio_wlock, 200);
#ifdef CONFIG_SDIO_CUST_SH
		}else{
...
#ifdef CONFIG_SDIO_CUST_SH
	if( strncmp(mmc_hostname(mmc),"mmc0",4) == 0){
		down(&sdio_suspendresume_sem);
		msmsdcc_state_suspend = 1;
		
		if(msmsdcc_state_idle != 0){
			msmsdcc_pm_idle_resume_lower(msmsdcc_idle_mmc0);
		}
		
		up(&sdio_suspendresume_sem);
	}
#endif
...
void msmsdcc_pm_idle_suspend(void){
	...
	down(&sdio_suspendresume_sem);
	if(msmsdcc_state_suspend != 0){
		up(&sdio_suspendresume_sem);
		return;
	}
	msmsdcc_state_idle = 1;
	up(&sdio_suspendresume_sem);
