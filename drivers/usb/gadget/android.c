...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (!strcmp(f->name, "rndis")) {
		if (enable) {
			/* skip notification caused by disconnect */
			_android_dev->cdev->mute_switch = 1;

			dev->default_mode = dev->current_mode;
			dev->current_mode = D_SH_ANDROID_USB_MODE_N_RNDIS;
		...
		if (dev->current_mode == D_SH_ANDROID_USB_MODE_N_CDC_2) {
			return;
		}
