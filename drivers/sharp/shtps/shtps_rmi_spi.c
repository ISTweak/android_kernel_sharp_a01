/* drivers/sharp/shtps/shtps_rmi_spi.c
 *
...
//#define SHTPS_FACETOUCH_OFF_DETECT_ENABLE
...
#define SHTPS_BOOT_AUTOREZERO_ENABLE
#define SHTPS_HRTIMER_POLLING_ENABLE
#define SHTPS_DRAGTHRESH_XY_INDEPEND
//#define SHTPS_REZERO_DELAYED_WORK_ENABLE
//#define SHTPS_DROP_EVENT_LIST_ENABLE

#if defined( SHTPS_LOG_EVENT_ENABLE )
	#define SHTPS_LOG_OUTPUT_SWITCH_ENABLE
#endif /* #if defined( SHTPS_LOG_EVENT_ENABLE ) */

#if defined( CONFIG_MACH_DECKARD_AS50 )
	#define SHTPS_ACTIVEMODE_ENABLE
	#define SHTPS_AUTOREZERO_SINGLE_FINGER_ENABLE
	#define SHTPS_AUTOREZERO_NO_AUTOCAL_ENABLE
	#define SHTPS_POSITION_OFFSET_ENABLE
	#if defined( SHTPS_BOOT_FWUPDATE_ENABLE )
	#include "shtps_fw_tm1737-004.h"
...
//	#define SHTPS_AUTOREZERO_NO_AUTOCAL_ENABLE
	#define SHTPS_POSITION_OFFSET_ENABLE
	#if defined( SHTPS_BOOT_FWUPDATE_ENABLE )
	#include "shtps_fw_tm1745-001.h"
	#endif /* #if defined( SHTPS_BOOT_FWUPDATE_ENABLE ) */
	#define SHTPS_FACETOUCH_OFF_DETECT_ENABLE
	#define SHTPS_FACETOUCH_DETECT_ENABLE
#endif

#if defined( SHTPS_DRAGTHRESH_XY_INDEPEND )
	#define SHTPS_DRAG_THRESH_VAL_X_1ST_VALUE			6
	#define SHTPS_DRAG_THRESH_VAL_Y_1ST_VALUE			6
	#define SHTPS_DRAG_THRESH_VAL_X_1ST_MULTI_VALUE		12
	#define SHTPS_DRAG_THRESH_VAL_Y_1ST_MULTI_VALUE		12
#else
	#define SHTPS_DRAG_THRESH_VAL_1ST_VALUE				6
#endif /* #if deinfed( SHTPS_DRAGTHRESH_XY_INDEPEND ) */
...
#define SHTPS_EVENT_POLL_INTERVAL			20
		...
		static int SHTPS_DRAG_THRESH_VAL_X_1ST = 		SHTPS_DRAG_THRESH_VAL_X_1ST_VALUE;
		static int SHTPS_DRAG_THRESH_VAL_X_2ND = 		1;
		static int SHTPS_DRAG_THRESH_VAL_X_1ST_MULTI =	SHTPS_DRAG_THRESH_VAL_X_1ST_MULTI_VALUE;
		static int SHTPS_DRAG_THRESH_VAL_X_2ND_MULTI =	1;
		static int SHTPS_DRAG_THRESH_VAL_Y_1ST = 		SHTPS_DRAG_THRESH_VAL_Y_1ST_VALUE;
		static int SHTPS_DRAG_THRESH_VAL_Y_2ND = 		1;
		static int SHTPS_DRAG_THRESH_VAL_Y_1ST_MULTI =	SHTPS_DRAG_THRESH_VAL_Y_1ST_MULTI_VALUE;
		static int SHTPS_DRAG_THRESH_VAL_Y_2ND_MULTI =	1;
		static int SHTPS_DRAG_THRESH_RETURN_TIME =		250;
	#else
		#define SHTPS_DRAG_THRESH_VAL_X_1ST				SHTPS_DRAG_THRESH_VAL_X_1ST_VALUE
		#define SHTPS_DRAG_THRESH_VAL_X_2ND				1
		#define SHTPS_DRAG_THRESH_VAL_X_1ST_MULTI		SHTPS_DRAG_THRESH_VAL_X_1ST_MULTI_VALUE
		#define SHTPS_DRAG_THRESH_VAL_X_2ND_MULTI		1
		#define SHTPS_DRAG_THRESH_VAL_Y_1ST				SHTPS_DRAG_THRESH_VAL_Y_1ST_VALUE
		#define SHTPS_DRAG_THRESH_VAL_Y_2ND				1
		#define SHTPS_DRAG_THRESH_VAL_Y_1ST_MULTI		SHTPS_DRAG_THRESH_VAL_Y_1ST_MULTI_VALUE
		#define SHTPS_DRAG_THRESH_VAL_Y_2ND_MULTI		1
		#define SHTPS_DRAG_THRESH_RETURN_TIME			250
	#endif /* #if defined( SHTPS_POLLING_DRAG_THRETHOLD_VARIABLE ) */
#else
	#if defined( SHTPS_POLLING_DRAG_THRETHOLD_VARIABLE )
		static int SHTPS_DRAG_THRESH_VAL_1ST =		SHTPS_DRAG_THRESH_VAL_X_1ST_VALUE;
		static int SHTPS_DRAG_THRESH_VAL_2ND =			1;
		static int SHTPS_DRAG_THRESH_VAL_1ST_MULTI =	8;
		static int SHTPS_DRAG_THRESH_VAL_2ND_MULTI =	1;
		static int SHTPS_DRAG_THRESH_RETURN_TIME =		250;
	#else
		#define SHTPS_DRAG_THRESH_VAL_1ST				SHTPS_DRAG_THRESH_VAL_Y_1ST_VALUE
		#define SHTPS_DRAG_THRESH_VAL_2ND				1
		#define SHTPS_DRAG_THRESH_VAL_1ST_MULTI			8
		#define SHTPS_DRAG_THRESH_VAL_2ND_MULTI			1
		#define SHTPS_DRAG_THRESH_RETURN_TIME			250
	#endif /* #if defined( SHTPS_POLLING_DRAG_THRETHOLD_VARIABLE ) */
#endif /* #if defined( SHTPS_DRAGTHRESH_XY_INDEPEND ) */
...
#if defined( SHTPS_DROP_EVENT_LIST_ENABLE )
static u16 shtps_drop_event_list[][2] = {
	{  0, 907},
	{479, 907},
	{0xFFFF, 0xFFFF}
};

#define SHTPS_IS_DROP_EVENT_POS(buf, idx) \
	(F11_DATA_XPOS((buf)) == shtps_drop_event_list[idx][0] && F11_DATA_YPOS((buf)) == shtps_drop_event_list[idx][1])

#endif /* #if defined( SHTPS_DROP_EVENT_LIST_ENABLE ) */
...
#if defined( SHTPS_HRTIMER_POLLING_ENABLE )
	struct hrtimer				polling_timer;
	struct work_struct			polling_work;
#else
	struct delayed_work 		polling_work;
#endif /* #if defined( SHTPS_HRTIMER_POLLING_ENABLE ) */
...
#if defined( SHTPS_REZERO_DELAYED_WORK_ENABLE )
	struct hrtimer				rezero_delayed_timer;
	struct work_struct			rezero_delayed_work;
#endif /* #if defined( SHTPS_REZERO_DELAYED_WORK_ENABLE ) */
};
...
static void shtps_rezero(struct shtps_rmi_spi *ts)
{
...
#endif
#else
#if defined( SHTPS_AUTOREZERO_NO_AUTOCAL_ENABLE )
	u8  val;
	...
	SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps] fw rezero execute\n"); )
	shtps_rmi_write(ts, ts->map.fn11.commandBase, 0x01);
#endif /* #if defined( SHTPS_AUTOREZERO_NO_AUTOCAL_ENABLE ) */
#endif /* #if 0 */
}
...
static void shtps_autorezero_disable(struct shtps_rmi_spi *ts)
{
	...
	shtps_rmi_write(ts, ts->map.fn01.ctrlBase, val | 0x20);
#else
	if(ver >= SHTPS_AUTO_CALIB_CTRL_ENABLE_VER){
		shtps_rmi_read(ts, 0xF0, &val, 1);
		shtps_rmi_write(ts, 0xF0, val | 0x01);
	}else{
		shtps_rmi_read(ts, ts->map.fn01.ctrlBase, &val, 1);
...
#if defined( SHTPS_FACETOUCH_OFF_DETECT_ENABLE )
...
#endif /* #if defined( SHTPS_FACETOUCH_OFF_DETECT_ENABLE ) */
...
#else
static void shtps_rec_notify_time(struct shtps_rmi_spi *ts, int index)
{
	ts->touch_state.drag_timeout[index][0] = jiffies + msecs_to_jiffies(SHTPS_DRAG_THRESH_RETURN_TIME);
}

static int shtps_chk_notify_time(struct shtps_rmi_spi *ts, int index)
{
	if(time_after(jiffies, ts->touch_state.drag_timeout[index][0])){
		return -1;
	}
	return 0;
}
...
#else
static int shtps_get_dragstep(struct shtps_rmi_spi *ts, int type, int fingers)
{
	int dragStep;
	
	if(type == SHTPS_DRAG_THRESHOLD_1ST){
		dragStep = (fingers <= 1)? SHTPS_DRAG_THRESH_VAL_1ST : SHTPS_DRAG_THRESH_VAL_1ST_MULTI;
	}else{
		dragStep = (fingers <= 1)? SHTPS_DRAG_THRESH_VAL_2ND : SHTPS_DRAG_THRESH_VAL_2ND_MULTI;
	}
...
#else
	dragStep1st = shtps_get_dragstep(ts, SHTPS_DRAG_THRESHOLD_1ST, numOfFingers);
#endif /* #if defined( SHTPS_DRAGTHRESH_XY_INDEPEND ) */
...
#else
		dragStepCur = shtps_get_dragstep(ts, ts->touch_state.dragStep[i][0], numOfFingers);
		if(info->fingers[i].state == 0x01){
			diff_x = shtps_get_diff(info->fingers[i].x, ts->report_info.fingers[i].x, shtps_scale_x);
			diff_y = shtps_get_diff(info->fingers[i].y, ts->report_info.fingers[i].y, shtps_scale_y);
			diff_cx= shtps_get_diff(info->fingers[i].x, ts->center_info.fingers[i].x, shtps_scale_x);
			...
			if((diff_x >= dragStepCur || diff_y >= dragStepCur) &&
					ts->report_info.fingers[i].state == 0x01)
			{
				if(diff_cx >= dragStep1st || diff_cy >= dragStep1st){
					*event = SHTPS_EVENT_DRAG;
					if(ts->report_info.fingers[i].state != 0){
						ts->touch_state.dragStep[i][0] = SHTPS_DRAG_THRESHOLD_2ND;
					}
					shtps_rec_notify_time(ts, i);
					ts->center_info.fingers[i].x = info->fingers[i].x;
					ts->center_info.fingers[i].y = info->fingers[i].y;

				}else if(shtps_chk_notify_time(ts, i) == 0){
					*event = SHTPS_EVENT_DRAG;
					if(ts->report_info.fingers[i].state != 0){
						ts->touch_state.dragStep[i][0] = SHTPS_DRAG_THRESHOLD_2ND;
					}
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	if(F11_QUERY_HASGESTURE1(ts->map.fn11.query.data) == 0){
		...
		shtps_rmi_read(ts, ts->map.fn01.dataBase + 29, &buf[29], 2);
	}
...
#if defined( SHTPS_DROP_EVENT_LIST_ENABLE )
	{
		...
		for(idx = 0;shtps_drop_event_list[idx][0] != 0xFFFF;idx++){
			if((buf[2] & 0x03) == 0x01){
				if(SHTPS_IS_DROP_EVENT_POS(&buf[4], idx)){
					buf[2] &= 0xFC;
					memset(&buf[4], 0, 5);
					printk("[shtps][test] drop finger0 event. (%3d, %3d)\n", 
								F11_DATA_XPOS((&buf[4])), F11_DATA_YPOS((&buf[4])));

				}
			}
			if(((buf[2] & 0x0C) >> 2) == 0x01){
				if(SHTPS_IS_DROP_EVENT_POS(&buf[9], idx)){
					buf[2] &= 0xF3;
					memset(&buf[9], 0, 5);
					printk("[shtps][test] drop finger1 event. (%3d, %3d)\n", 
								F11_DATA_XPOS((&buf[9])), F11_DATA_YPOS((&buf[9])));
				}
			}
			if(((buf[2] & 0x30) >> 4) == 0x01){
				if(SHTPS_IS_DROP_EVENT_POS(&buf[14], idx)){
					buf[2] &= 0xCF;
					memset(&buf[14], 0, 5);
					printk("[shtps][test] drop finger2 event. (%3d, %3d)\n", 
								F11_DATA_XPOS((&buf[14])), F11_DATA_YPOS((&buf[14])));
				}
			}
			if(((buf[2] & 0xC0) >> 6) == 0x01){
				if(SHTPS_IS_DROP_EVENT_POS(&buf[19], idx)){
					buf[2] &= 0x3F;
					memset(&buf[19], 0, 5);
					printk("[shtps][test] drop finger3 event. (%3d, %3d)\n", 
								F11_DATA_XPOS((&buf[19])), F11_DATA_YPOS((&buf[19])));
				}
			}
			if((buf[3] & 0x03) == 0x01){
				if(SHTPS_IS_DROP_EVENT_POS(&buf[24], idx)){
					buf[3] &= 0xFC;
					memset(&buf[24], 0, 5);
					printk("[shtps][test] drop finger4 event. (%3d, %3d)\n", 
								F11_DATA_XPOS((&buf[24])), F11_DATA_YPOS((&buf[24])));
				}
			}
		}
	}
...
static int shtps_statef_active_enter(struct shtps_rmi_spi *ts, int param)
{
	if(param == SHTPS_STATE_WAIT_READY){
		shtps_init_param(ts);
		#if defined( SHTPS_BOOT_AUTOREZERO_ENABLE )
		...
		#endif /* #if defined( SHTPS_BOOT_AUTOREZERO_ENABLE ) */
...
#if defined( SHTPS_POLLING_DRAG_THRETHOLD_VARIABLE )
	if((int)arg != 0){
		SHTPS_DRAG_THRESH_VAL_1ST = (int)(arg & 0xFF);
		SHTPS_DRAG_THRESH_VAL_2ND = (int)((arg >> 0x08) & 0xFF);
		SHTPS_DRAG_THRESH_VAL_1ST_MULTI = (int)((arg >> 0x10) & 0xFF);
		SHTPS_DRAG_THRESH_VAL_2ND_MULTI = (int)((arg >> 0x18) & 0xFF);
	}
#endif /* #if defined( SHTPS_POLLING_DRAG_THRETHOLD_VARIABLE ) */
	return SHTPS_DRAG_THRESH_VAL_1ST | 
		   ((SHTPS_DRAG_THRESH_VAL_2ND << 0x08) & 0x0000FF00) |
		   ((SHTPS_DRAG_THRESH_VAL_1ST_MULTI << 0x10) & 0x00FF0000) |
		   ((SHTPS_DRAG_THRESH_VAL_2ND_MULTI << 0x18) & 0xFF000000);
}
#endif /* #if defined( SHTPS_DRAGTHRESH_XY_INDEPEND ) */
...
#if defined( SHTPS_VIRTUALKEY_ENABLE )
	__set_bit(KEY_PROG1, input_dev->keybit);
#endif /* #if defined( SHTPS_VIRTUALKEY_ENABLE ) */
