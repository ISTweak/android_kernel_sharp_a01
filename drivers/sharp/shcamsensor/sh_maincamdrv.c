...
//			}
//			if (param.w_data[0] == CAM_GPIO_HI) {
//				ret = vreg_set_level(camdrv_vreg, 1200);
//				ret = vreg_enable(camdrv_vreg);
//				udelay(SH_MAINCAM_DELAY);
//			}
//			else {
...
static int sh_maincam_drv_read_adjust_data( void __user *argp )
{
	...
	do {
		...
		if( sh_smem_common != NULL ) {
			memcpy((unsigned char *)param.r_data,
				   (unsigned char *)sh_smem_common->sh_camImgAdjustData,
				   sizeof(sh_smem_common->sh_camImgAdjustData));
		}
		else {
			ret = -EFAULT;
			break;
