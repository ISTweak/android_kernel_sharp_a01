...
printk(KERN_INFO "[%s] sdata->sensor_reset=%d power=%d\n", __func__, sdata->sensor_reset, power);
    if(1 == power) {
    ...
    printk(KERN_INFO "t8ev4_probe_init_sensor finishes\n");
    return ret;
}
...
#if 0   /* debug */
            pTmp = (unsigned char*)sh_smem_common->sh_camOtpData;
            for(ii = 0; ii < 1024; ii+=8) {
                printk(KERN_INFO "OTP Data VCM 0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n",
                pTmp[ii+0],pTmp[ii+1],pTmp[ii+2],pTmp[ii+3],
                pTmp[ii+4],pTmp[ii+5],pTmp[ii+6],pTmp[ii+7]);
            }
#endif
        }
        else {
