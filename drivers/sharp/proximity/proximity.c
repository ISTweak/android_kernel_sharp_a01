...
static void PROX_Irq_workfunc(WorkStruct *work)
{
	...
	#endif	
	#ifdef CONFIG_PROXIMITY_MODEL_TYPE1
	if(MVO == 1)
	{
		/* HYS = H'20 */
		bData = 0x20;
	...
	#endif
	#ifdef CONFIG_PROXIMITY_MODEL_TYPE2
	switch(rev)
	{
	case HW_ES1:
		if(MVO == 1)
...
static int PROX_ConfigGPIO(drv_data *poProximityRec)
{
...
#ifdef CONFIG_PROXIMITY_REV_TYPE1
	switch(rev)
	{
	case HW_ES0:
	case HW_ES1:
...
#endif
#ifdef CONFIG_PROXIMITY_REV_TYPE2
	switch(rev)
	{
	case HW_ES0:
	case HW_ES1:
	...
	gpio_tlmm_config(GPIO_CFG(poProximityRec->irq_gpio,   0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
